<?php
  ...
  # inicializar erro
	...
	{
		$regionais->assignIdentifier($_POST['id']);
		
     # recuperar os campos
    $regionais = $regionais->recuperarCampos($regionais, $_POST, 'editar');
    ...
    $erro = $regionais->validar($regionais);    
		...
		if (count($erro) == 0)
		{
			$regionais->modified_at = date('Y-m-d H:i:s');
			$regionais->status = 1;
	...
	{
		setMensagem("Registro inválido");
		header('Location: '. URl . 'regionais/index'); exit;
	}
