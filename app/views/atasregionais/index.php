<?php
	...
	$objIndex = $atasregionaisController->index($pg, $queryString, $sqlString); 
	
	# extrair as variaveis do array
	extract($objIndex);
	
	# total de registros
	$total = count($atasregionais);
			 	...
			 	<div class="conteudo-rg">					
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabela-rg">
							...
							<tr class="menu-rg">
								...
								<th align="left" width="500">Título</th>
							...
							<tr class="listar-rg <?=$bg?>">
								...
								<td><?=formataData($atasregionais[$i]['data_ata'])?></td>	
								
								<td><?=$atasregionais[$i]['numero']?></td>
								
								<td align="left">
									<strong><a href="<?=URL?>atasregionais/editar/?id=<?=$atasregionais[$i]['id']?>" title="<?=$atasregionais[$i]['titulo']?>"><?=$atasregionais[$i]['titulo']?></a></strong>
								</td>						
