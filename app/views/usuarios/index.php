<?php
	...
	$objIndex = $usuariosController->index($pg, $queryString, $sqlString); 
			...
			<div class="meio-conteudo-borda">
				<div class="meio-conteudo">
					...
					<div class="conteudo-ft">
						...
						<form name="filtros" method="get" action="<?=URL . 'usuarios/index/'?>">
							<div class="bloco-ft">
								<label>Nome</label>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabela-rg">
							...
							<?
								...
								for($i=0; $i<$total; $i++)
								{
									...
									{
										$status = '<img onClick="javascript: atualizarStatus(0, '.$usuarios[$i]['id'].', \'usuarios\');" src="'.IMG_URL.'status-ok.png" alt="Alterar Status" />';
									}
									elseif($usuarios[$i]['status'] == 0)
									{
										$status = '<img onClick="javascript: atualizarStatus(1, '.$usuarios[$i]['id'].', \'usuarios\');" src="'.IMG_URL.'status-no.png" alt="Alterar Status" />';
									}
							?>
							<tr class="listar-rg <?=$bg?>">
								
								<td align="left">
									<strong><a href="<?=URL?>usuarios/editar/?id=<?=$usuarios[$i]['id']?>" title="<?=$usuarios[$i]['nome']?>"><?=$usuarios[$i]['nome']?></a></strong>
								</td>
								
								<td><?=$usuarios[$i]['created_at']?></td>
								
								<!-- status -->
								<td id="status-<?=$usuarios[$i]['id']?>"><?=$status?></td>
								
								<!-- -->
								<!--<td class="no-borda-right" align="right"><a href="visualizar.html" title="Visualizar Registro"><img src="<?=IMG_URL?>visualizar.png" /></td>-->
								<td class="no-borda-right"><a href="<?=URL?>usuarios/remover/?id=<?=$usuarios[$i]['id']?>" title="Remover Registro"><img src="<?=IMG_URL?>remover.png" /></td>
