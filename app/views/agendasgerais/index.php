<?php
	...
	$objIndex = $agendasgeraisController->index($pg, $queryString, $sqlString); 
	
	# extrair as variaveis do array
	extract($objIndex);
	
	# total de registros
	$total = count($agendasgerais);
        ...
        <div id="meio">
			...
			<div class="meio-conteudo-borda">
				<div class="meio-conteudo">
			   	...
			   	<form name="filtros" method="get" action="<?=URL . 'agendasgerais/index/'?>">
			 	...
			 	<div class="conteudo-rg">					
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabela-rg">
							...
							<tr class="menu-rg">
								...
								<th align="left" width="450">Título</th>
							...
							<tr class="listar-rg <?=$bg?>">
								...
								<td><?=formataData($agendasgerais[$i]['data_agenda'])?></td>	
								
								<td><?=$agendasgerais[$i]['local']?></td>
								
								<td align="left">
									<strong><a href="<?=URL?>agendasgerais/editar/?id=<?=$agendasgerais[$i]['id']?>" title="<?=$agendasgerais[$i]['titulo']?>"><?=$agendasgerais[$i]['titulo']?></a></strong>
								</td>						
								
								<!-- -->
								<td class="no-borda-right" align="right"><a href="<?=URL?>agendasgerais/visualizar/?id=<?=$agendasgerais[$i]['id']?>" title="Visualizar Registro"><img src="<?=IMG_URL?>visualizar.png" /></td>
								<td class="no-borda-right"><a href="<?=URL?>agendasgerais/excluir/?id=<?=$agendasgerais[$i]['id']?>" title="Remover Registro"><img src="<?=IMG_URL?>remover.png" /></td>
