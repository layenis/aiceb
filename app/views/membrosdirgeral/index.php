<?php
	...
	$objIndex = $membrosdirgeralController->index($pg, $queryString, $sqlString); 
	
	# extrair as variaveis do array
	extract($objIndex);
	
	# total de registros
	$total = count($membrosdirgeral);
			...
			<div class="meio-conteudo-borda">
				<div class="meio-conteudo">
					...
					<div class="conteudo-ft">
						...
						<form name="filtros" method="get" action="<?=URL . 'membrosdirgeral/index/'?>">
							...
							<div class="bloco-ft">
								<strong><a href="<?=URL?>membrosdirgeral/historico" title="Histórico de membros">Visualizar histórico de membros da diretoria geral</a>
							</div>
			 	...
			 	<div class="conteudo-rg">					
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabela-rg">
						...
						<tr class="listar-rg <?=$bg?>">
							...
							<td align="left">
								<strong><a href="<?=URL?>membrosdirgeral/editar/?id=<?=$membrosdirgeral[$i]['id']?>" title="<?=$membrosdirgeral[$i]['nome']?>"><?=$membrosdirgeral[$i]['nome']?></a></strong>
							</td>						
													
							<td align="left"><?=$membrosdirgeral[$i]['igreja_id']?></td>
							
							<td><?=$membrosdirgeral[$i]['funcao']?></td>								
							<!-- -->
							<td class="no-borda-right" align="right"><a href="<?=URL?>membrosdirgeral/visualizar/?id=<?=$membrosdirgeral[$i]['id']?>" title="Visualizar registro"><img src="<?=IMG_URL?>visualizar.png" /></td>
							<td class="no-borda-right"><a href="<?=URL?>membrosdirgeral/excluir/?id=<?=$membrosdirgeral[$i]['id']?>" title="Remover Registro"><img src="<?=IMG_URL?>remover.png" /></td>
