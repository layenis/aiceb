<?php
	...
	$objIndex = $submenusController->index($pg, $queryString, $sqlString); 
			...
			<div class="meio-conteudo-borda">
				<div class="meio-conteudo">
					...
					<div class="conteudo-ft">
						...
						<form name="filtros" method="get" action="<?=URL . 'submenus/index/'?>">
							<div class="bloco-ft">
								<label>Nome</label>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabela-rg">
							...
							<?
								...
								for($i=0; $i<$total; $i++)
								{
									...
									{
										$status = '<img onClick="javascript: atualizarStatus(0, '.$submenus[$i]['id'].', \'submenus\');" src="'.IMG_URL.'status-ok.png" alt="Alterar Status" />';
									}
									elseif($submenus[$i]['status'] == 0)
									{
										$status = '<img onClick="javascript: atualizarStatus(1, '.$submenus[$i]['id'].', \'submenus\');" src="'.IMG_URL.'status-no.png" alt="Alterar Status" />';
									}
							?>
							<tr class="listar-rg <?=$bg?>">
								
								<td><?=$submenus[$i]['posicao']?></td>
								
								<td align="left">
									<strong><a href="<?=URL?>menus/editar/?id=<?=$submenus[$i]['menu_id']?>" title="<?=$submenus[$i]['nome']?>"><?=select('Menus', 'nome', $submenus[$i]['menu_id'])?></a> > <a href="<?=URL?>submenus/editar/?id=<?=$submenus[$i]['id']?>" title="<?=$submenus[$i]['nome']?>"><?=$submenus[$i]['nome']?></a></strong>
								</td>
								
								<td><?=$submenus[$i]['created_at']?></td>
								
								<!-- status -->
								<td id="status-<?=$submenus[$i]['id']?>"><?=$status?></td>
								
								<!-- -->
								<!--<td class="no-borda-right" align="right"><a href="visualizar.html" title="Visualizar Registro"><img src="<?=IMG_URL?>visualizar.png" /></td>-->
								<td class="no-borda-right"><a href="<?=URL?>submenus/remover/?id=<?=$submenus[$i]['id']?>" title="Remover Registro"><img src="<?=IMG_URL?>remover.png" /></td>
