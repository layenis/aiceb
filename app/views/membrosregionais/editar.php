<?php
	...
	{
		$membrosregionais->assignIdentifier($_POST['id']);
			
		# recuperar os campos
		$membrosregionais = $membrosregionais->recuperarCampos($membrosregionais, $_POST, 'editar');
		...
		if (count($erro) == 0)
		{
			...
			$membrosregionais->modified_at = date('Y-m-d H:i:s');
			
			if (!empty($membrosregionais->final_gestao))
				$membrosregionais->status = 0;
