<?php
	...
	if(isset($_POST['action']))
	{
		...
		$menus->assignIdentifier(post('id'));
		
		# recuperar os campos -> retorna um objeto
		$menus = $menus->recuperarCampos($menus, $_POST, 'editar');
		
		# validação -> retorna um array
		$erro = $menus->validar($menus);
		
		if(count($erro) == 0)
		{			
			$menus->modified_at = date('Y-m-d H:m:s');
			
		    # salvar
			$menusController->salvar($menus);
		}
		else
		{
			# mensagem de erro
	...
	{
		# pesquisa pelo id
		$menus = $menusController->buscaPorId($id);
